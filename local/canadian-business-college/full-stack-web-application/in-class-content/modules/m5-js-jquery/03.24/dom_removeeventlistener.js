...
my_button.addEventListener("click", async function () {
...
// an external named function so that remove event listener can be associated with it
function remove_event() {
  console.log("Mouse enter cancelled");
  my_heading.style.backgroundColor = "black";
  my_heading.style.color = "white";
  // remove event listener removing the event after the first time it has worked
  my_heading.removeEventListener("mouseenter", remove_event);
}

my_heading.addEventListener("mouseenter", remove_event);
