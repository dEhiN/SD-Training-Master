...
app.get("/fetch-data", async function (req, res) {
    ...
    // to use projection without filter, empty filter has to be kept, plus if key does not exist for the data it will be empty response for those data
    let db_data = await my_collection.find({}, { projection: { hobby: 1, _id: 0 } }).toArray();
