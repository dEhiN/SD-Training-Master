...
// a function that will be used to send out email in case of error in the server
function send_error_log_email(error_body) {

    let email_content = {
        // your own email address
        from: "ababhaybarthwal@gmail.com",
        // email address for where you want to send email
        to: "abarthwal@canadianbusinesscollege.com",
        subject: "Error in the server",
        text: error_body
    }
    ...
    fs.readFile("test.txt", function (error, data) {
        if (error) {
            if (error.code == "ENOENT") {
                send_error_log_email("no such file or directory " + error.path)
            }
            else {
                send_error_log_email(error.code + " " + error.syscall + " " + error.path)
            }
        }
        else {
            res.send(data);
